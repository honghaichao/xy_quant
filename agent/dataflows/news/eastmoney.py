...
from agent.dataflows.news.base import BaseNewsSource, NewsItem


def debug_news_log() -> bool:
    """控制新闻模块日志开关。关闭以避免 akshare 内部 regex bug 的噪音。"""
    return False
...
class EastMoneyNews(BaseNewsSource):
    ...
    def get_stock_news(self, symbol: str, limit: int = 20,
        ...
        try:
            ...
            # akshare 的 regex bug 或其他内部错误不打印堆栈
            if debug_news_log():
