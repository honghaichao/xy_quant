...
def _get_conn(read_only: bool = False):
    """获取数据库连接"""
    return duckdb.connect(_get_db_path(), read_only=read_only)
...
def init_pipeline(pipeline_name: str, date: str) -> str:
    ...
    conn = _get_conn()
    try:
        # pipeline_id = 名称_日期 (无随机后缀，避免 get_or_create_pipeline 匹配困难)
        pipeline_id = f"{pipeline_name}_{date}"

        # 检查是否已有相同 pipeline_id，有则直接返回
        existing = conn.execute(
            "SELECT COUNT(*) FROM data_pipeline_run WHERE pipeline_id = ?",
            [pipeline_id]
        ).fetchone()[0]
        if existing > 0:
            return pipeline_id
...
def get_or_create_pipeline(pipeline_name: str, date: str) -> Dict[str, Any]:
    ...
    conn = _get_conn()
    try:
        # 尝试查找指定日期的流水线（格式: daily_20260323）
        pipeline_id = f"{pipeline_name}_{date}"
        row = conn.execute("""
