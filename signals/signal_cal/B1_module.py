...
import logging
import numpy as np
from signals.signal_cal.basic_module import calculate_知行多空线_arr
from signals.signal_cal.basic_module import calculate_知行短期趋势线_arr
...
def calculate_b1_score(indicators):
    ...
    logger.debug("=== B1各条件得分详情 ===")
    logger.debug(f"股票代码code: {code}")
    logger.debug(f"条件1得分(DIF>=0): {条件1得分}")
    logger.debug(f"条件2得分(C>MA60): {条件2得分}")
    logger.debug(f"条件3得分(涨幅-2~1.8): {条件3得分}")
    logger.debug(f"条件4得分(振幅<7): {条件4得分}")
    logger.debug(f"条件5得分(振幅<4): {条件5得分}")
    logger.debug(f"条件6得分(RSI<20/23): {条件6得分}")
    logger.debug(f"条件7得分(趋势线): {条件7得分}")
    logger.debug(f"条件8得分(地量30-10天): {条件8得分}")
    logger.debug(f"条件9得分(地量8-18天): {条件9得分}")
    logger.debug(f"条件10得分(地量20天): {条件10得分}")
    logger.debug(f"条件11得分(倍量+阳线): {条件11得分}")         
    logger.debug(f"条件12得分(倍量+阴线): {条件12得分}")
    logger.debug(f"条件13得分(近期倍量+阴线): {条件13得分}")
    logger.debug(f"条件14得分(50/20日涨跌和): {条件14得分}")
    logger.debug(f"条件15得分(涨跌量对比): {条件15得分}")
    logger.debug(f"条件16得分(BBI>前20日BBI): {条件16得分}")
    logger.debug(f"条件17得分(支撑位距离): {条件17得分}")
    logger.debug(f"条件18得分(趋势线偏离): {条件18得分}")
    logger.debug(f"条件19得分(创业板趋势线): {条件19得分}")
    logger.debug(f"条件20得分(多空线偏离): {条件20得分}")
    logger.debug(f"条件21得分(价格区间): {条件21得分}")
    logger.debug(f"条件22得分(DIF背离): {条件22得分}")
    logger.debug(f"条件23得分(放量上涨): {条件23得分}")
    logger.debug(f"条件24得分(区间振幅): {条件24得分}")
    logger.debug(f"条件25得分(跳空阳线): {条件25得分}")
    logger.debug(f"条件26得分(知行线波动): {条件26得分}")
    logger.debug(f"条件27得分(成交量变化): {条件27得分}")
    logger.debug(f"条件28得分(趋势线下降): {条件28得分}")
    logger.debug(f"条件29得分(十字星): {条件29得分}")
    logger.debug(f"条件30得分(J值高位): {条件30得分}")
    logger.debug(f"条件31得分(缩量连续下跌): {条件31得分}")
    logger.debug(f"条件32得分(关键K): {条件32得分}")
    logger.debug(f"条件33得分(连续大长阳): {条件33得分}")
    logger.debug(f"条件34得分(大长阴): {条件34得分}")
    logger.debug(f"条件35得分(RSI多头): {条件35得分}")
    logger.debug(f"条件36得分(涨停次数): {条件36得分}")
    logger.debug(f"条件37得分(开盘压制): {条件37得分}")
    logger.debug(f"条件38得分(次高点): {条件38得分}")
    logger.debug(f"条件39得分(无关键K): {条件39得分}")
    logger.debug(f"\n>>> B1总分: {b1_score}")
