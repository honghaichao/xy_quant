...
def calculate_s1_score(indicators: Dict) -> float:
    ...
    logger.debug("=== S1各条件得分详情 ===")
    logger.debug(f"股票代码code: {indicators['code']}")
    logger.debug(f"条件1得分: {条件1评分}")
    logger.debug(f"条件2得分: {条件2评分}")
    logger.debug(f"加分1: {加分1}")
    logger.debug(f"加分2: {加分2}")      
    logger.debug(f"\n>>> S1总分: {s1_score}")
