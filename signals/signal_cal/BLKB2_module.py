...
def check_暴力K(indicators):
    """
    检查是否出现暴力K（与天宫暴力K策略V1一致）
    暴力K条件:
    1. C > REF(C, 1)
    2. VOL > 参考成交量 * 1.8
    3. 涨跌幅 > 4
    ...
    K线长度 = high - low
    上影线 = high - max(close, open_p)

    上影线比例 = 上影线 / K线长度 if K线长度 > 0 else 0

    if len(close_arr) < 2 or len(volume_arr) < 2:
        return False
    
    # 计算参考成交量（使用昨日成交量作为基准）
    # 如果昨日成交量异常小（<=今日/8），则用前日成交量
    if len(volume_arr) >= 2:
        if volume_arr[-2] <= volume / 8:
            参考成交量 = volume_arr[-3] if len(volume_arr) > 2 else volume
        else:
            参考成交量 = volume_arr[-2]  # 昨日成交量
    else:
        参考成交量 = volume
    ...
    if len(close_arr) >= 10 and len(open_arr) >= 10:
        prev_close_ref = close_arr[-2]
        open_9 = open_arr[-9]
        if open_9 != 0:
            前9日涨幅 = (prev_close_ref - open_9) / open_9 * 100
            cond5 = 前9日涨幅 < 4
        else:
            cond5 = True
    else:
    ...
    if len(close_arr) >= 5 and len(open_arr) >= 5:
        ...
        open_4 = open_arr[-4]
        if open_4 != 0:
            前4日涨幅 = (prev_close_ref - open_4) / open_4 * 100
            cond6 = 前4日涨幅 < 4
        else:
            cond6 = True
    else:
    ...
    condition = cond1 and cond2 and cond3 and cond4 and cond5 and cond6 and cond7

    if condition:
        logger.debug(f"\n=== BLK暴力K得分详情 ===")
        logger.debug(f"触发条件: cond1={cond1}, cond2={cond2}, cond3={cond3}, cond4={cond4}, cond5={cond5}, cond6={cond6}, cond7={cond7}")
        logger.debug(f"涨跌幅: {涨跌幅:.2f}%")
        logger.debug(f"上影线比例: {上影线比例:.2%}" if K线长度 > 0 else "上影线比例: N/A")
        logger.debug(f"成交量倍数: {volume/参考成交量:.2f}x" if 参考成交量 > 0 else "成交量倍数: N/A")
    
    return condition

def check_倍量柱(indicators):
    """
    检查是否出现倍量柱（与通达信公式一致）
    ...
    cond1 = volume > 参考成交量 * 1.8
    cond2 = close > open_price
    cond3 = close > prev_close

    condition = cond1 and cond2 and cond3
    
    return condition

def check_J拐头向上(indicators):
    """
    检查J值是否对比前两天出现拐头向上
    条件: 今日J > 昨日J AND 昨日J < 前日J (即J值先下降后上升)
    """
    ...
    if len(close_arr) < 4 or len(high_arr) < 10 or len(low_arr) < 10:
        return False
...
def calculate_b2_score(indicators):
    ...
    logger.debug(f"\n=== B2各条件得分详情 ===")
    logger.debug(f"B2触发: {B2触发}")
    logger.debug(f"得分1(J值): {得分1}")
    logger.debug(f"得分2(DIF>=0): {得分2}")
    logger.debug(f"得分3(均线多头): {得分3}")
    logger.debug(f"得分4(C>知行多空线): {得分4}")
    logger.debug(f"得分5(KDJ): {得分5}")
    logger.debug(f"得分6(C>BBI): {得分6}")
    logger.debug(f"得分7(RSI): {得分7}")
    logger.debug(f"得分8(上影线): {得分8}")
    logger.debug(f"得分9(上下影线): {得分9}")
    logger.debug(f"得分10(成交量): {得分10}")
    logger.debug(f"得分11(高点): {得分11}")
    logger.debug(f"得分12(阳量): {得分12}")
    logger.debug(f"得分13(最大阳量): {得分13}")
    logger.debug(f"得分14(突破): {得分14}")
    logger.debug(f"得分15(DIF背离): {得分15}")
    logger.debug(f"得分16(量能高点): {得分16}")
    logger.debug(f"得分17(近期量能): {得分17}")
    logger.debug(f"得分18(上证): {得分18}")
    logger.debug(f"得分19(放量): {得分19}")
    logger.debug(f"得分20(关键K): {得分20}")
    logger.debug(f"得分21(暴力K): {得分21}")
    logger.debug(f"得分22(RSI多头): {得分22}")
    logger.debug(f"得分23(缩量下跌): {得分23}")
    logger.debug(f"得分24(量能新高): {得分24}")
    logger.debug(f"得分25(J上升): {得分25}")
    logger.debug(f"得分26(大长阴): {得分26}")
    logger.debug(f"得分27(低量能): {得分27}")
    logger.debug(f"得分28(十字星): {得分28}")
    logger.debug(f"得分29(条件8/9): {得分29}")
    logger.debug(f"得分30(趋势线): {得分30}")
    logger.debug(f"得分31(反包): {得分31}")
    logger.debug(f"\n>>> B2总分: {b2_score}")
