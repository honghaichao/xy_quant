...
def calculate_scb_signal(indicators,blk_signal, dl_basic_history):
    ...
    logger.debug(f"\n=== SCB买入信号详情 ===")
    logger.debug(f"过去5天满足地量基础条件: {has_dl_basic}")
    logger.debug(f"今日暴力K: {blk_signal}")
    logger.debug(f"多头发散: {多头信号}")
    logger.debug(f"SCB信号: {scb_signal}")
    logger.debug(f"SCB评分: {scb_score}")
