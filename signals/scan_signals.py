...
from signals.signal_cal.SCB_module import (
    calculate_dl_score, check_dl_basic_condition,
    calculate_blk_signal, calculate_scb_signal,
)
from signals.signal_cal.DZ30_module import calculate_倍量柱_arr, check_前20日非阴, check_长短期KD
...
# 配置路径 - 使用 xy_quant 的 config
import os as _os
from config.settings import settings
PROJECT_ROOT = _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))
DB_PATH = _os.path.join(PROJECT_ROOT, settings.duckdb_path.lstrip("./"))
...
def get_stock_list() -> List[Dict]:
    """获取全市场股票列表（从 PostgreSQL stock_basic 表，若不可用则从 DuckDB daily_bar 去重）"""
    try:
    ...
    # Fallback: get distinct ts_codes from DuckDB daily_bar
    conn = get_db_connection()
    try:
        df = conn.execute("""
...
def get_stock_data(code: str, trading_date: str, days: int = DATA_DAYS) -> Optional[pd.DataFrame]:
    """获取股票历史数据（从 xy_quant daily_bar 表）"""
    conn = get_db_connection()
    try:
...
def scan_signals(trading_date: str, workers: int = DEFAULT_WORKERS) -> Dict[str, Any]:
    ...
    # 信号计算时压制详细日志（条件得分只写 debug）
    for _mod_name in ['signals.signal_cal.B1_module', 'signals.signal_cal.B2_module',
                      'signals.signal_cal.BLKB2_module', 'signals.signal_cal.SCB_module',
                      'signals.signal_cal.S1_module', 'signals.signal_cal.basic_module',
                      'signals.signal_cal.DZ30_module']:
        _m = logging.getLogger(_mod_name)
        _m.setLevel(logging.WARNING)
